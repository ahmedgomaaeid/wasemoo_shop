...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-3xl">
    <div class="px-6 py-5 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
        <div>
            <h2 class="text-xl font-semibold text-gray-900">Edit Section</h2>
            <p class="text-sm text-gray-500 mt-1">Make changes to "{{ $section->name }}"</p>
        </div>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" 
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-3 transition-colors">{{ old('description', $section->description) }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-sm text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    @if($section->image_path)
                        <div class="mb-4 flex items-start">
                            <div class="relative group">
                                <img src="{{ asset('storage/'.$section->image_path) }}" alt="{{ $section->name }}" class="h-32 w-32 object-cover rounded-lg border border-gray-200 shadow-sm">
                            </div>
                        </div>
                    @endif
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-gray-500 mt-2">
                                Leave empty to keep the current image.<br>PNG, JPG, WEBP up to 2MB
                            </p>
                        </div>
                    </div>
                    @error('image_path')
